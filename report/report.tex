...
每个簇内汇总各门店的样本，训练一个浅层 XGBoost。门店聚类的结果见图 \ref{fig:dtw}。
...
单店建模因样本过少而过拟合，整体表现最差。各模型在四个指标上的排名见图 \ref{fig:rank}。

\section{可视化结果分析}

变量分布图（图 \ref{fig:dist}）显示 \texttt{Weekly\_Sales} 明显右偏，取 $\log(1+y)$ 后接近对称，说明对数变换是合适的；
气温呈双峰，反映南北门店的气候差异。

相关热力图（图 \ref{fig:corr}）显示，各外生变量与销售额的线性相关都很弱，说明关系以非线性为主，
...
DTW 距离矩阵（图 \ref{fig:dtw}）在按簇排序后呈现清晰的分块结构，轮廓系数在 $K=5$ 处最高，
说明门店可以稳定地分为五类，且与门店规模层次对应。

损失曲线（图 \ref{fig:loss}）中训练与验证 RMSE 同步下降并很快趋稳，两者间距不大，未出现明显过拟合。
...
末段留出的预测值与真实值对比及残差分布见图 \ref{fig:pred}。
...
\noindent
图 \ref{fig:shap} 的纵轴列出了全部参与建模的特征。除 \texttt{Holiday\_Flag}、\texttt{Fuel\_Price}、
